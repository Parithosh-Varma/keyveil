...
async function keysCommand(sub, _args, opts) {
  ...
  if (sub === "list") {
    ...
    printTable(
      (keys || []).map((k) => ({
        ...k,
        uses: k.max_uses == null ? `${k.uses ?? 0}/unlimited` : `${k.uses ?? 0}/${k.max_uses}`,
      })),
      ["id", "name", "key_prefix", "scopes", "uses", "expires_at", "revoked_at"]
    );
  } else if (sub === "create") {
    ...
    if (opts.singleUse) body.max_uses = 1;
    ...
    console.log(`created key ${res.id} (${res.prefix}) scopes=${(res.scopes || []).join(",")}${res.max_uses === 1 ? " single-use" : ""}`);
