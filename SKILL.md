# vault-use skill — paste this into OpenCode every time
...
Auth: Authorization: Bearer $VAULT_AGENT_TOKEN (already in env, never print it)
```
...
4. On 401/403/429 stop and say: "vault auth failed / scope denied / IP not allowlisted / rate limited" — do not retry with a different key, do not dump headers.
5. Every proxy call is audit-logged per user. Keep args minimal.

Example (curl, token hidden):
```bash
curl -s -H "Authorization: Bearer $VAULT_AGENT_TOKEN" \
